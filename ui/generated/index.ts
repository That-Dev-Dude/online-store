...
export type ProductQuery = {
  product: Pick<Product, 'id' | 'title' | 'price' | 'shortDescription' | 'description'> & {
    options?: Maybe<
      Array<
        Pick<ProductOptionGroup, 'id' | 'name' | 'productType'> & {
          options: Array<Pick<ProductOption, 'id' | 'option' | 'description'>>
        }
      >
    >
    images: Array<Pick<ProductImage, 'url' | 'altText'>>
    displayImage: Pick<ProductImage, 'url' | 'altText'>
    reviews: Array<Pick<Review, 'id' | 'title' | 'author' | 'isVerified' | 'date' | 'review' | 'productId' | 'stars'>>
  }
}
